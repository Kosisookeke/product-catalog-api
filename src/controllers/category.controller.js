...
exports.createCategory = async (req, res) => {
  try {
    ...
    if (existingCategory) return sendError(res, 400, 'Category name already exists');
...
exports.updateCategory = async (req, res) => {
  try {
    ...
    if (!updatedCategory) return sendError(res, 404, 'Category not found');

    logger.info(`Category updated: ${id}`);
    sendSuccess(res, 200, updatedCategory);
  } catch (err) {
...
exports.deleteCategory = async (req, res) => {
  try {
    ...
    if (!deletedCategory) return sendError(res, 404, 'Category not found');
